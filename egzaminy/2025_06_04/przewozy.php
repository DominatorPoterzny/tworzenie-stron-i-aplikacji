<!DOCTYPE html>
<html lang="pl">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Firma Przewozowa</title>
    <link rel="stylesheet" href="styles.css">
</head>
<body>
    <header>
        <h1>Firma przewozowa Półdarmo</h1>
    </header>
    <nav>
        <a href="kw1.png">kwerenda1</a>
        <a href="KW2.png">kwerenda2</a>
        <a href="kw3.png">kwerenda3</a>
        <a href="kw4.png">kwerenda4</a>
    </nav>
    <main>
        <section class="lewa">
            <h2>Zadania do wykonania</h2>
            <form action="przewozy.php" method="post">
            <table>
                <!-- skrypt2 -->
                
                <tr>
                    <th>Zadanie do wykonania</th>
                    <th>Data realizacji</th>
                    <th>Akcja</th>
                </tr>
                <!-- skrypt1 -->

                <?php  
                    $connect = mysqli_connect('localhost','root','','przewozy');
                    $result = mysqli_query($connect,"SELECT id_zadania,zadanie,data FROM zadania;");
                    while($row = mysqli_fetch_assoc($result)){
                       echo" <tr>";
                       echo"     <td>$row[zadanie]</td>";
                       echo"     <td>$row[data]</td>";
                       echo"     <td><a href='przewozy.php?usun=$row[id_zadania]' name='usun_zadanie'>Usuń</a></td>";
                       echo" </tr>";
                    }
                ?>
                <?php
                    if(isset($_POST['dodaj'])){
                    $result2 = mysqli_query($connect,"INSERT INTO zadania (zadanie,data,osoba_id) VALUES ('$_POST[zadanie]','$_POST[data]',1)");
                    }
                ?>
                <?php
                    if(isset($_GET['usun'])){
                        $result3 = mysqli_query($connect,"DELETE FROM zadania WHERE id_zadania = $_GET[usun];");
                    }
                ?>
                <?php mysqli_close($connect) ?>
            </table>
            <label for="zadanie">Zadanie do wykonania</label>
            <input type="text" name="zadanie"><br>
            <label for="data">Data realizacji</label>
            <input type="date" name="data">
            <input type="submit" value="Dodaj" name="dodaj">
            </form>
        </section>
        <section class="prawa">
            <img src="./pliki11/auto.png"  alt="auto formowe">
            <h3>Nasza specjalność</h3>
            <ul>
                <li>Przeprowadzki</li>
                <li>Przewóz mebli</li>
                <li>przesyłki gabarytowe</li>
                <li>Wynajem pojazdów</li>
                <li>Zakupy towarów</li>
            </ul>
        </section>
    </main>
    <footer>
        <p>Stronę wykonał: Potężny Dominator</p>
    </footer>
</body>
</html>